...
interface DrawerLink { href: string; label: string; icon: string; roles?: UserRole[]; }

const CLIENTE_LINKS: DrawerLink[] = [
...
export function DrawerMenu({ open, onClose }: Props) {
  ...
  const links = role === "cliente" ? CLIENTE_LINKS : STAFF_LINKS;
  ...
  return (
    <>
      ...
      <aside className="drawer-menu" role="dialog" aria-label="Menu de navegação">
        ...
        <nav>
          {links.filter((l) => !l.roles || l.roles.includes(role)).map((link) => (
            <a key={link.href} href={link.href}
              className={`drawer-link${path === link.href ? " active" : ""}`}
              data-icon={link.icon} onClick={onClose}>
              {link.label}
            </a>
          ))}
        </nav>
        {user && (
          <>
            ...
            <button className="drawer-link" data-icon="log-out"

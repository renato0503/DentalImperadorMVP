...
const ITEMS: NavItem[] = [
  { href: "/meu-painel", label: "Início", icon: "home", roles: ["admin", "manager", "operator", "cliente"] },
    ...
    <nav className="mobile-nav" role="tablist" aria-label="Navegação principal">
      {visible.map((item) => (
        <a key={item.href} href={item.href}
          ...
          data-icon={item.icon} role="tab" aria-selected={path === item.href} aria-label={item.label}>
          {item.label}
        </a>
      ))}
    </nav>
